...
function compute_DI(mJ::Matrix{Float64}, C::Matrix{Float64}, q::Integer = 21)
    Nq = size(mJ, 1)
    size(mJ, 2) == Nq || throw(ArgumentError("mJ matrix must be square"))
    size(C) == size(mJ) || throw(ArgumentError("incompatible mJ and C matrices"))
    s = q - 1
    Nq % s == 0 || throw(ArgumentError("incompatible size of mJ with q"))
